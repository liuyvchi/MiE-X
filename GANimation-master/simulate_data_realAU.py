...
class MorphFacesInTheWild:
    ...
    def morph_file(self, img_path, expresion):
        ...
        dir_path = os.path.join('./EmotioNet/generated_multi_faces_realAU_5repeats', dir_name)
...
def main():
    ...
    data = pd.read_csv(apex_label_path, header=None, sep = ' ')
    ...
    au_activation_repeat = []
    with open('./EmotioNet/selected_imgs_all.csv', 'rb') as f:
        with open('simulate_multi_labels_realAU5w_5repeats.csv', 'wb') as f_labels:
            for line in f:
                line = line.strip('\n')
                ...
                for i in range(3):
                    ...
                    if person_num % 5 == 0:
                        au_activation_repeat = au_activation
                    au_activation = au_activation_repeat
                    expression = {'type': label, 'au': au_activation}
                    morph.morph_file(img_path, expression)
