...
def build_model_3pic(num_classes=3):
    model = GAN_npic(fc=[512 * 3, 128, 32, 3])
...
def main():
    ...
    for fold_num in range(3):
        train_datasets.append(
            GAN_loader(args, train_data_paths[fold_num], train_label_paths[fold_num], transform=transform, Micro_set=True)
        )
        vali_datasets.append(
            GAN_loader(args, vali_data_paths[fold_num], vali_label_paths[fold_num], transform=transform, Micro_set=True)
        )
        test_datasets.append(
            GAN_loader(args, test_data_paths[fold_num], test_label_paths[fold_num], transform=transform, Micro_set=True)
        )
    ...
    for i in range(3):
        ...
        optimizers.append(optim.Adam([
            ...
            {'params': model['resnet'][2].parameters(), 'lr': args.lr_E},
