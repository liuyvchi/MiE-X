    ...
    for i, sample in enumerate(train_dataloader):
        ...
        if i % print_freq == 0:
            print(  'Train:\t'
                    ...
                    'Loss: {loss:.4f}\t'.format(
                        epoch, i + 1, len(train_dataloader), acc=accuracy, c_loss=classification_loss, d_loss=domain_loss, loss=loss))
...
def validate(validate_dataloader, model, criterion):
    ...
    print('Validation:\t'
          ...
          'Loss: {loss:.4f}\t'
          'Recall: {recall:.4f}\t'
          'f1_score: {f1_score:.4f}\t'.format( acc=accuracy, c_loss=total_loss_c, d_loss=total_loss_d, loss=total_loss, recall=recall, f1_score=f1_score))
...
def main():
    ...
    parser.add_argument('--DATA_ROOT', type=str, default='/home/user/Yuchi/iccv2019/GANimation-master/', help='path to generated faces')
    ...
    parser.add_argument('--nepochs_decay', type=int, default=10, help='# of epochs to linearly decay learning rate to zero')
    ...
    train_dataset = New_loader(args, args.train_data_path, sample_num = 5000, mix_type = 1, transform=transform, Micro_set=False)
    validation_dataset = New_loader(args, args.validation_data_path, sample_num = 30000, mix_type = 2, transform=transform, Micro_set=False)
    ...
    validation_loader = DataLoader(validation_dataset, batch_size=args.batch_size, shuffle=True, pin_memory=True, num_workers=args.num_workers)
    test_loader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=True, pin_memory=True, num_workers=args.num_workers)
    ...
    for epoch in range(args.nepochs_no_decay + args.nepochs_decay):
        ...
        vali_f1_score, vali_recall, vali_acc = validate(validation_loader, model, criterion)
        ...
        print ('best_f1_score_vali:', best_vali_f1s, 'choosen_test_f1_score:', choosen_test_f1, 'choosen_test_recall:', choosen_test_recall, 'choosen_test_acc:', choosen_test_acc)
