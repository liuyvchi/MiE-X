...
def train(train_dataset, train_dataloader, model, criterion, optimizer, epoch, ft= False,print_freq=50):
    for i in range(2):
        model['resnet'][i].train()
    model['classifier'].train()
    model['classifier_ft'].train()
    ...
    for i, sample in enumerate(train_dataloader):
        ...
        if ft:
            classification_output = model['classifier_ft'](features)
        else:
        ...
        nn.utils.clip_grad_norm_(model['classifier_ft'].parameters(), Clip_Norm, norm_type=2)
...
def test(test_dataloader, model, criterion):
    ...
    with torch.no_grad():
        for i, sample in enumerate(test_dataloader):
            ...
            matrix = confusion_matrix(label.cpu(), preds.cpu(), labels=[0, 1, 2, 3,4,5])
            FP += matrix.sum(axis=0) - np.diag(matrix)
            FN += matrix.sum(axis=1) - np.diag(matrix)
            TP += np.diag(matrix)
            TN += matrix.sum() - (FP + FN + TP)
    ...
    print('test:\t'
          ...
          'uar_score: {uar_score:.4f}\t'.format(acc=accuracy, c_loss=total_loss_c, loss=total_loss,
                                                f1_score=f1_score, uar_score=recall))
...
def main():
    ...
    parser.add_argument('--DATA_ROOT', type=str, default='/home/yuchi/micro-expression/MER',
                        help='path to generated faces')
    parser.add_argument('--SIMU_DATA_ROOT', type=str, default='/home/yuchi/micro-expression/GANimation',
                        help='path to generated faces')
    parser.add_argument('--Loading_path', type=str, default="/home/yuchi/micro-expression/MER/Loading_file_new/5w_real+ck+prior_fold/",help='path to training data')
    parser.add_argument('--Loading_path_f', type=str, default="/home/yuchi/micro-expression/MER/Loading_file_new/MMEW_5fold/", help='path to fine tuning data')

    parser.add_argument('--batch_size', type=int, default=32, help='input batch size')
    parser.add_argument('--num_workers', type=int, default=4, help='number of workers')
    parser.add_argument('--nepochs_no_decay', type=int, default=0, help='# of epochs at starting learning rate')
    parser.add_argument('--nepochs_decay', type=int, default=0,
                        help='# of epochs to linearly decay learning rate to zero')
    parser.add_argument('--nepoches_fineT', type=int, default=80, help='# of epochs at starting learning rate')
    ...
    for fold_num in range(5):
        train_data_paths.append(os.path.join(args.Loading_path, 'simulate_fold{}_data.txt'.format(fold_num%3)))
        # train_label_paths.append(os.path.join(args.Loading_path, 'simulate_fold{}_label0.txt'.format(fold_num)))

    test_data_paths = []
    test_label_paths = []
    for fold_num in range(5):
        test_data_paths.append(os.path.join(args.Loading_path_f, 'MMEW_fold{}_test_data.txt'.format(fold_num)))
        test_label_paths.append(os.path.join(args.Loading_path_f, 'MMEW_fold{}_test_label.txt'.format(fold_num)))

    train_datasets = []
    test_datasets = []
    for fold_num in range(5):
        train_datasets.append(
            New_loader(args, train_data_paths[fold_num],  sample_num = 3000, mix_type = 3, transform=transform, Micro_set=False)
        )
    ...
    train_label_paths_f = []
    for fold_num in range(5):
        train_data_paths_f.append(os.path.join(args.Loading_path_f, 'MMEW_fold{}_train_data.txt'.format(fold_num)))
        train_label_paths_f.append(os.path.join(args.Loading_path_f, 'MMEW_fold{}_train_label.txt'.format(fold_num)))

    train_datasets_f = []
    for fold_num in range(5):
        train_datasets_f.append(
            Micro_loader(args, train_data_paths_f[fold_num], train_label_paths_f[fold_num], transform=transform,
                         Micro_set=True)
        )

    train_loaders_f = []
    for fold_num in range(5):
        train_loaders_f.append(
            DataLoader(train_datasets_f[fold_num], batch_size=args.batch_size, shuffle=True, pin_memory=True,
                       num_workers=args.num_workers))
    models = []
    optimizers = []
    for i in range(5):
        ...
        model['classifier_ft'].cuda()
        model['discriminator'].cuda()

        model['classifier'] = model['classifier'].apply(weight_init)
        model['discriminator'] = model['discriminator'].apply(weight_init)

        # model['resnet'][0].load_state_dict(torch.load('log/simu_fineT/fold2_encoder0.pt'))
        # model['resnet'][1].load_state_dict(torch.load('log/simu_fineT/fold2_encoder1.pt'))
        ...
        optimizers.append(optim.Adam([
            ...
            {'params': model['classifier_ft'].parameters(), 'lr': args.lr_C},
            # {'params': model['discriminator'].parameters(), 'lr': args.lr_D},
        ], weight_decay=args.weight_decay))
        models.append(model)

    fold_total = 5
    best_f1s = np.zeros(fold_total)
    best_acc = np.zeros(fold_total)

    choosen_test_score = np.zeros(fold_total)
    choosen_test_recall = np.zeros(fold_total)
    choosen_test_acc = np.zeros(fold_total)
    choosen_test_FP = [0, 0, 0, 0, 0]
    choosen_test_FN = [0, 0, 0, 0, 0]
    choosen_test_TP = [0, 0, 0, 0, 0]
    choosen_test_TN = [0, 0, 0, 0, 0]


    for epoch in range(args.nepochs_no_decay + args.nepochs_decay + args.nepoches_fineT):

        print('epoch:', epoch, "################################################################")

        for fold_num in range(fold_total):
            if epoch < args.nepochs_no_decay + args.nepochs_decay:
                ...
                start = time.time()
                train(train_datasets[fold_num], train_loaders[fold_num], models[fold_num], criterion,
                      optimizers[fold_num], epoch, ft = False)
                f1_score_test, recall_test, accuracy_test, FP, FN, TP, TN = test(test_loaders[fold_num],
                                                                                 models[fold_num], criterion)
            else:
                print('fold number', fold_num, "===============================================")
                if epoch %10 == 0 and epoch > 0:
                    for i in range(len(optimizers[fold_num].param_groups)):
                        optimizers[fold_num].param_groups[i]['lr'] = optimizers[fold_num].param_groups[i]['lr']*0.8
                        print(optimizers[fold_num].param_groups[i]['lr'])
                start = time.time()
                train(train_datasets_f[fold_num], train_loaders_f[fold_num], models[fold_num], criterion,
                      optimizers[fold_num], epoch, ft = True)
                f1_score_test, recall_test, accuracy_test, FP, FN, TP, TN = test(test_loaders[fold_num],
                                                                                 models[fold_num], criterion)
                torch.save(models[fold_num]['resnet'][0].state_dict(),
                           "log/branches/mmew_fold" + str(fold_num) + "_encoder0.pt")
                torch.save(models[fold_num]['resnet'][1].state_dict(),
                           "log/branches/mmew_fold" + str(fold_num) + "_encoder1.pt")
                torch.save(models[fold_num]['classifier'].state_dict(),
                           "log/branches/mmew_fold" + str(fold_num) + "_classifier.pt")

            print("cost for the last epoch:", time.time() - start)
            if accuracy_test >= best_acc[fold_num]:
                best_acc[fold_num] = accuracy_test
            ...
            # print('best_f1_score_vali for fold{}'.format(fold_num), best_acc[fold_num])
            print('best_accuracy for fold{}'.format(fold_num), best_acc[fold_num])
        print('choosen_f1_score_vali', np.mean(best_f1s), 'choosen_f1_score_test', np.mean(choosen_test_score), 'choosen_recall_test', np.mean(choosen_test_recall), "choosen_accuracy_test:", np.mean(choosen_test_acc))
            # test(validation_loader, model, criterion)

    print("best_acc:")
    print(best_acc)
    final_FP, final_FN, final_TP, final_TN = 0, 0, 0, 0
    for i in range(choosen_test_FP):
