...
def main():
    ...
    parser.add_argument('--Loading_path_f', type=str, help='path to fine tuning data')
    ...
    parser.add_argument('--nepoches_fineT', type=int, default=0, help='# of epochs at starting learning rate')
    ...
    for fold_num in range(3):
        # train_data_paths.append(os.path.join(args.Loading_path, 'simulate_fold0_data.txt'))
    ...
    for fold_num in range(3):
        train_datasets.append(
            New_loader(args, train_data_paths[fold_num],  sample_num = 15000, mix_type = 3, transform=transform, Micro_set=False)
        )
    ...
    for fold_num in range(3):
        train_datasets_f.append(
            Micro_loader(args, train_data_paths_f[fold_num], train_label_paths_f[fold_num], target_name=None,transform=transform,
                         Micro_set=True)
        )
    ...
    for fold_num in range(3):
        ...
        # 'log/simu_fineT/fold2_encoder0.pt'
        # model['resnet'][0].load_state_dict(torch.load('log/branches/SAMM_fold{}_encoder0.pt'.format(fold_num)))
        # model['resnet'][1].load_state_dict(torch.load('log/branches/SAMM_fold{}_encoder1.pt'.format(fold_num)))
        # # model['classifier'].load_state_dict(torch.load('log/branches/mmew_fold4_classifier.pt'))
    ...
    for epoch in range(args.nepochs_no_decay + args.nepochs_decay + args.nepoches_fineT):
        ...
        for fold_num in range(3):
            if epoch < args.nepochs_no_decay + args.nepochs_decay:
                ...
                if epoch == args.nepochs_no_decay + args.nepochs_decay:
                    for i in range(len(optimizers[fold_num].param_groups)):
                        optimizers[fold_num].param_groups[i]['lr'] = args.lr_E
                        print(optimizers[fold_num].param_groups[i]['lr'])
                ...
                torch.save(models[fold_num]['resnet'][0].state_dict(),
                           "log/branches/front_fold" + str(fold_num) + "_encoder0.pt")
                torch.save(models[fold_num]['resnet'][1].state_dict(),
                           "log/branches/front_fold" + str(fold_num) + "_encoder1.pt")
                torch.save(models[fold_num]['classifier'].state_dict(),
                           "log/branches/front_fold" + str(fold_num) + "_classifier.pt")
        ...
        print('choosen_f1_score_vali', np.mean(best_f1s), 'choosen_f1_score_test', np.mean(choosen_test_score), 'choosen_recall_test', np.mean(choosen_test_recall), "best_accuracy_test:", np.mean(choosen_test_acc))
