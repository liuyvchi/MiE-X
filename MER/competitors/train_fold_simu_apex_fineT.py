...
from Utils.Dataloader_GAN import GAN_loader, New_loader, Micro_loader, New_loader_flo, Micro_loader_flo
...
def test(test_dataloader, model, criterion):
    ...
    with torch.no_grad():
        for i, sample in enumerate(test_dataloader):
            ...
            tmp_FP = matrix.sum(axis=0) - np.diag(matrix)
            tmp_FN = matrix.sum(axis=1) - np.diag(matrix)
            tmp_TP = np.diag(matrix)
            FP += tmp_FP
            FN += tmp_FN
            TP += tmp_TP
            TN += matrix.sum() - (tmp_FP + tmp_FN + tmp_TP)
...
def build_model_2pic(num_classes=3):
    model = GAN_npic(fc=[512 * 2, 128, 32, 6])
...
def main():
    ...
    parser.add_argument('--Loading_path', type=str,
                        default="/home/yuchi/micro-expression/MER/Loading_file_new/5w_real+ck+prior_fold/",
                        help='path to training data')
    parser.add_argument('--Loading_path_f', type=str,
                        default="/home/yuchi/micro-expression/MER/Loading_file_new/macro_micro_3fold/",
                        help='path to fine tuning data')

    parser.add_argument('--batch_size', type=int, default=32, help='input batch size')
    parser.add_argument('--num_workers', type=int, default=4, help='number of workers')
    parser.add_argument('--nepochs_no_decay', type=int, default=10, help='# of epochs at starting learning rate')
    ...
    for fold_num in range(3):
        train_data_paths.append(os.path.join(args.Loading_path, 'simulate_fold{}_data.txt'.format(fold_num)))
        # train_label_paths.append(os.path.join(args.Loading_path, 'simulate_fold{}_label0.txt'.format(fold_num)))

    test_data_paths = []
    test_label_paths = []
    for fold_num in range(3):
        test_data_paths.append(os.path.join(args.Loading_path_f, 'Micro_fold{}_data2.txt'.format(fold_num)))
        test_label_paths.append(os.path.join(args.Loading_path_f, 'Micro_fold{}_label2.txt'.format(fold_num)))

    train_datasets = []
    test_datasets = []
    for fold_num in range(3):
        train_datasets.append(
            New_loader_flo(args, train_data_paths[fold_num],  sample_num = 15000, mix_type = 3, transform=transform, Micro_set=False)
        )
        test_datasets.append(
            Micro_loader_flo(args, test_data_paths[fold_num], test_label_paths[fold_num], transform=transform, Micro_set=True)
        )
    ...
    train_label_paths_f = []
    for fold_num in range(3):
        train_data_paths_f.append(os.path.join(args.Loading_path_f, 'Micro_fold{}_data01.txt'.format(fold_num)))
        train_label_paths_f.append(os.path.join(args.Loading_path_f, 'Micro_fold{}_label01.txt'.format(fold_num)))

    train_datasets_f = []
    for fold_num in range(3):
        train_datasets_f.append(
            Micro_loader_flo(args, train_data_paths_f[fold_num], train_label_paths_f[fold_num], transform=transform,
                         Micro_set=True)
        )
    ...
    for i in range(3):
        ...
        # model['resnet'].load_state_dict(torch.load('/home/yuchi/micro-expression/MER/competitors/log/flo/fold0_apex.pt'))
        # model['classifier'].load_state_dict(torch.load('/home/yuchi/micro-expression/MER/competitors/log/flo/fold0_classifier_apex.pt'))
    ...
    for epoch in range(args.nepochs_no_decay + args.nepochs_decay + args.nepoches_fineT):
        ...
        for fold_num in range(fold_total):
            if epoch < args.nepochs_no_decay + args.nepochs_decay:
                ...
                train(train_datasets[fold_num], train_loaders[fold_num], models[fold_num], criterion,
                      optimizers[fold_num], epoch)
                f1_score_test, recall_test, accuracy_test, FP, FN, TP, TN = test(test_loaders[fold_num],
                                                                                 models[fold_num], criterion)
                # torch.save(models[fold_num]['resnet'].state_dict(),
                #            "log/flo/fold" + str(fold_num) + "_encoder.pt")
                # torch.save(models[fold_num]['classifier'].state_dict(),
                #            "log/flo/fold" + str(fold_num) + "_classifier.pt")
            else:
                print('fold number', fold_num, "===============================================")
                if epoch % 10 == 0 and epoch > 0:
                    for i in range(len(optimizers[fold_num].param_groups)):
                        optimizers[fold_num].param_groups[i]['lr'] = optimizers[fold_num].param_groups[i]['lr'] * 0.9
                        print(optimizers[fold_num].param_groups[i]['lr'])
                start = time.time()
        ...
        print('choosen_f1_score_vali', np.mean(best_f1s), 'choosen_f1_score_test', np.mean(choosen_test_score),
              'choosen_recall_test', np.mean(choosen_test_recall), "choosen_accuracy_test:", np.mean(choosen_test_acc))
        # test(validation_loader, model, criterion)

    print("best_acc:")
    print(best_acc)
    final_FP, final_FN, final_TP, final_TN = 0, 0, 0, 0
    for i in range(fold_total):
    ...
    final_f1_s = np.ones([fold_total])
    for f in range(fold_total):
        if deno[f] != 0:
