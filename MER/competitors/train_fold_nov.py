...
def train(train_dataset, train_dataloader, model, criterion, optimizer, epoch, print_freq=5):
    ...
    for i, sample in enumerate(train_dataloader):
        ...
        output_encoder = []
        for k in range(2):
            output_encoder.append(model['backbone'][k](input[:, k]))
        ...
        # Clip_Norm = 1
        # nn.utils.clip_grad_norm_(model['backbone'][0].parameters(), Clip_Norm, norm_type=2)
        # nn.utils.clip_grad_norm_(model['backbone'][1].parameters(), Clip_Norm, norm_type=2)
...
def build_model_2pic(num_classes=3):
    model = model_npic(fc=[512 * 2, 128, 32, 3])
...
def main():
    ...
    for i in range(3):
        ...
        optimizers.append(optim.Adam([
            {'params': model['backbone'][0].parameters(), 'lr': args.lr_E},
            {'params': model['backbone'][1].parameters(), 'lr': args.lr_E},
