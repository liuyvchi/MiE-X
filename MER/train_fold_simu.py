...
def train(train_dataset, train_dataloader, model, criterion, optimizer, epoch, print_freq=100):
    for i in range(2):
...
def main():
    ...
    vali_label_paths = []
    for fold_num in range(3):
        vali_data_paths.append(os.path.join(args.Loading_path, 'Micro_fold{}_data2.txt'.format(fold_num)))
        vali_label_paths.append(os.path.join(args.Loading_path, 'Micro_fold{}_label2.txt'.format(fold_num)))
    ...
    for fold_num in range(3):
        train_datasets.append(
            New_loader(args, train_data_paths[fold_num],  sample_num = 3000, mix_type = 2, transform=transform, Micro_set=False)
        )
        vali_datasets.append(
            Micro_loader(args, vali_data_paths[fold_num], vali_label_paths[fold_num], transform=transform, Micro_set=True)
        )
    ...
    for epoch in range(args.nepochs_no_decay + args.nepochs_decay):
        ...
        for fold_num in range(3):
            ...
            f1_score_vali, recall_vali, accuracy_vali= validate(vali_loaders[fold_num], models[fold_num], criterion)
