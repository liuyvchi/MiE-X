...
def train(train_dataset, train_dataloader, model, criterion, optimizer, epoch, print_freq=5):
    for i in range(2):
    ...
    for i, sample in enumerate(train_dataloader):
        input, label, domain_label, selected_label = sample['images'], sample['label'], sample['domain_label'], sample['selected_label']
        input, label, domain_label, selected_label = input.cuda(), label.cuda(), domain_label.cuda(), selected_label.cuda()
        batch_size = input.size()[0]
        input = input[selected_label]
        domain_label = domain_label[selected_label]
...
def main():
    ...
    for fold_num in range(3):
        train_datasets.append(
            Micro_loader(args, train_data_paths[fold_num], train_label_paths[fold_num], "CASMEII", transform=transform, Micro_set=True)
        )
