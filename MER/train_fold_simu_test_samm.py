...
def train(train_dataset, train_dataloader, model, criterion, optimizer, epoch, print_freq=50):
    ...
    for i, sample in enumerate(train_dataloader):
        ...
        # loss = domain_loss * 0.5 + classification_loss
...
def main():
    ...
    parser.add_argument('--SIMU_DATA_ROOT', type=str, default='/home/yuchi/micro-expression/GANimation/',
                        help='path to generated faces')
    parser.add_argument('--Loading_path_test', type=str, default="/home/yuchi/micro-expression/MER/Loading_file_new/samm_5fold/",help='path to training data')
    parser.add_argument('--Loading_path_train', type=str, default="/home/yuchi/micro-expression/MER/Loading_file_new/5w_real+ck+prior_fold/",help='path to training data')
    ...
    for fold_num in range(3):
        train_data_paths.append(os.path.join(args.Loading_path_train, 'simulate_fold{}_5classSAMM.txt'.format(fold_num)))
        # train_label_paths.append(os.path.join(args.Loading_path, 'simulate_fold{}_label0.txt'.format(fold_num)))

    test_data_paths = []
    test_label_paths = []
    for fold_num in range(3):
        test_data_paths.append(os.path.join(args.Loading_path_test, 'samm_fold{}_data1_5class.txt'.format(1)))
        test_label_paths.append(os.path.join(args.Loading_path_test, 'samm_fold{}_label1_5class.txt'.format(1)))
    ...
    for epoch in range(args.nepochs_no_decay + args.nepochs_decay):
        ...
        for fold_num in range(3):
            ...
            if f1_score_test >= best_f1s[fold_num]:
                ...
                torch.save(models[fold_num]['resnet'][0].state_dict(), "checkpoint/fold" + str(fold_num) + "_encoder0.pt")
                torch.save(models[fold_num]['resnet'][1].state_dict(), "checkpoint/fold" + str(fold_num) + "_encoder1.pt")
                torch.save(models[fold_num]['classifier'].state_dict(), "checkpoint/fold" + str(fold_num) + "_classifier.pt")
