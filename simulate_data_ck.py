...
class MorphFacesInTheWild:
    ...
    def morph_file(self, img_path, expresion):
        ...
        # dir_path = os.path.join('./EmotioNet/generated_multi_faces_ckAU_2040', dir_name)
        dir_path = os.path.join('./EmotioNet/generated_multi_faces_ckAU_5repeat', dir_name)
...
def main():
    ...
    CK_AU_PATH = os.path.join(Dataset_dir, 'ck_AU_out_static')


    apex_label_path = 'CK_apx_label.csv'
    apex_label_path = os.path.join(Dataset_dir, apex_label_path)
    data = pd.read_csv(apex_label_path, dtype = str, header=None, sep = ' ')
    ...
    with open('./EmotioNet/selected_imgs_all.csv', 'rb') as f:
        # with open('simulate_multi_labels_CKAU_2040.csv', 'wb') as f_labels:
        with open('simulate_multi_labels_CKAU_5repeat', 'wb') as f_labels:
            for line in f:
                line = line.strip('\n')
                ...
                for i in range(3):
                    ...
                    row = data[data[4] == str(i)].iloc[realAU_ids[i]]
                    dataset_type, subject, trail, num_apx, label = str(row[0]), str(row[1]), str(row[2]), int(row[3]), str(row[4])
