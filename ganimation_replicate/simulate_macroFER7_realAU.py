...
for emotion_label in range(3):
    for i in range(ID_numbers):
        ...
        au_activation_0 = np.zeros(17)
        au_activation_1 = np.zeros(17)
        potential_au = au_map[expression_type]
        for j in potential_au.keys():
            if random.uniform(0, 1) < potential_au[j]:
                au_activation_1[openface_map[j]] = random.uniform(0.5, 1.5)/5
        au_activation = [au_activation_0, au_activation_1]
        AU_MiEexp_pool[str(emotion_label)].append(au_activation)

print(AU_MiEexp_pool['0'][:1])
print(len(AU_MiEexp_pool['1']))
print(len(AU_MiEexp_pool['2']))

# ## store ingredients ####
# with open("AU_MiEexp_pool.pkl", "wb") as wf:
#     pickle.dump(AU_MiEexp_pool, wf)
...
# with open('AU_MiEexp_pool.pkl', 'rb') as rf:
#     AU_MiEexp_pool = pickle.load(rf)
#
...
tar_dir = 'output/generated5k_macro_ouluAU+mmewAU'
...
exp_tar_aus={'0':None, '1':None, '2': None}
...
for idx_global in range(5000):
    ...
    for emotion_label in range(3):
        ...
        # cur_gen_face_0 = numpy2im(cur_gen_faces[0])
        # cur_gen_face_1 = numpy2im(cur_gen_faces[1])
        #
        # # save image_0
        ...
        # saved_path_0 = os.path.join(saved_path_dir, '0.jpg')
        # cur_gen_face_0.save(saved_path_0)
        # # save image_1
        # saved_path_1 = os.path.join(saved_path_dir, '1.jpg')
        # cur_gen_face_1.save(saved_path_1)


        # ##### generate AU_mae (CK+) based MiEs ####
        # if sample_au:
        #     pool_size = len(mae_pool[str(emotion_label)])
        #     pool_sample_idx = idx % pool_size
        #     mae_name = mae_pool[str(emotion_label)][pool_sample_idx]
        #     subject, trail = mae_name[0], mae_name[1]
        #     trail_aus_path = os.path.join(CK_AU_PATH, subject, trail, trail + '.csv')
        #     frames_aus = pd.read_csv(trail_aus_path, header=None)
        #     mae_tar_aus_0[str(emotion_label)] = torch.tensor(frames_aus.iloc[1][5:22].values.astype(np.float)).unsqueeze(dim=0)
        #     prob = random.uniform(0.25, 0.5)
        #     num_apx = len(frames_aus)
        #     num_apx = int(num_apx * prob)
        #     mae_tar_aus_1[str(emotion_label)] = torch.tensor(frames_aus.iloc[1 + num_apx][5:22].values.astype(np.float)).unsqueeze(dim=0)
        #     mae_tar_aus[str(emotion_label)] = torch.cat((mae_tar_aus_0[str(emotion_label)], mae_tar_aus_1[str(emotion_label)]), dim=0)/5
        #
        # test_batch = {'src_img': img_tensor, 'tar_aus': mae_tar_aus[str(emotion_label)], 'src_aus': mae_tar_aus[str(emotion_label)],
        #               'tar_img': img_tensor}
        # test_model.feed_batch(test_batch)
        # test_model.forward()
        # cur_gen_faces = test_model.fake_img.detach().cpu().float().numpy()  # to be simple, only generate one face each time
        ...
        #
        # # save image_0
        # saved_path_dir = os.path.join(tar_dir, "%s_%s_%s" % (target_sample_dir, 'mae', str(emotion_label)))
        # if not os.path.exists(saved_path_dir):
        #     os.makedirs(saved_path_dir)
        # saved_path_0 = os.path.join(saved_path_dir,'0.jpg')
        # cur_gen_face_0.save(saved_path_0)
        ...
        ##### generate AU_mae (MMEW) based MiEs ####
        if sample_au:
            pool_size = len(mmew_pool[str(emotion_label)])
            pool_sample_idx = idx % pool_size
            trail_aus_path = mmew_pool[str(emotion_label)][pool_sample_idx]
            frames_aus = pd.read_csv(trail_aus_path, header=None)
            mae_tar_aus_0[str(emotion_label)] = torch.tensor(
                frames_aus.iloc[1][5:22].values.astype(np.float)).unsqueeze(dim=0)
            # prob = random.uniform(0.25, 0.5)
            prob = random.uniform(0.8, 1)
            num_apx = len(frames_aus)
            num_apx = int(num_apx * prob)
            mae_tar_aus_1[str(emotion_label)] = torch.tensor(
                frames_aus.iloc[num_apx-1][5:22].values.astype(np.float)).unsqueeze(dim=0)
            mae_tar_aus[str(emotion_label)] = torch.cat(
                (mae_tar_aus_0[str(emotion_label)], mae_tar_aus_1[str(emotion_label)]), dim=0) / 5

        test_batch = {'src_img': img_tensor, 'tar_aus': mae_tar_aus[str(emotion_label)],
                      'src_aus': mae_tar_aus[str(emotion_label)],
                      'tar_img': img_tensor}
        test_model.feed_batch(test_batch)
        test_model.forward()
        cur_gen_faces = test_model.fake_img.detach().cpu().float().numpy()  # to be simple, only generate one face each time
        ...
        ##### generate AU_mae (MMEW) based MiEs ####
        if sample_au:
            pool_size = len(oulu_pool[str(emotion_label)])
            pool_sample_idx = idx % pool_size
            trail_aus_path = oulu_pool[str(emotion_label)][pool_sample_idx]
            frames_aus = pd.read_csv(trail_aus_path, header=None)
            mae_tar_aus_0[str(emotion_label)] = torch.tensor(
                frames_aus.iloc[1][5:22].values.astype(np.float)).unsqueeze(dim=0)
            # prob = random.uniform(0.25, 0.5)
        ...
        # save image_0
        saved_path_dir = os.path.join(tar_dir, "%s_%s_%s" % (target_sample_dir, 'oulu', str(emotion_label)))
        if not os.path.exists(saved_path_dir):
            os.makedirs(saved_path_dir)
        saved_path_0 = os.path.join(saved_path_dir, '0.jpg')
        cur_gen_face_0.save(saved_path_0)
